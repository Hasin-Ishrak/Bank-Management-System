...
exports.createAccount = async (req, res) => {
    ...
    if (!user_id) {
        return res.status(400).json({ message: 'User ID is required to bind the account.' });
    }

    const depositAmount = initial_deposit ? parseFloat(initial_deposit) : 0.00;
    if (depositAmount < 0) {
    ...
    try {
        ...
        if (userExists.length === 0) {
            return res.status(444).json({ message: 'The specified User ID does not exist.' });
        }
        ...
        let isUnique = false;
        while (!isUnique) {
            const [duplicate] = await db.query('SELECT id FROM accounts WHERE account_number = ?', [accountNumber]);
...
exports.getAccountDetails = async (req, res) => {
    ...
    const targetUserId = req.user.role === 'Customer' ? req.user.id : req.params.userId;
...
exports.deactivateAccount = async (req, res) => {
    ...
    try {
        const [account] = await db.query('SELECT id FROM accounts WHERE account_number = ?', [accountNumber]);
        if (account.length === 0) {
            return res.status(404).json({ message: 'Target account number not found.' });
        }

        await db.query('UPDATE accounts SET status = "Inactive" WHERE account_number = ?', [accountNumber]);
        res.status(200).json({ message: `Account ${accountNumber} has been successfully deactivated.` });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Database failure during account adjustment.' });
    }
};